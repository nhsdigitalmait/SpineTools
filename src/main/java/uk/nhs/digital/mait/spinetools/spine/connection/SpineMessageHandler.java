...
import uk.nhs.digital.mait.spinetools.spine.messaging.EbXmlAcknowledgment;
...
/**
 * Thread to handle the receipt of an ebXML message and its acknowledgement. The
 ...
 * after any acknowledgement has been returned.
 *
...
public class SpineMessageHandler
    ...
    public void run() {
        ...
        try {
            ...
            //
            if (clen == -1) {
                SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.messaging.sendable.message", "Failed to get inbound content length, aborting");
                socket.close();
                return;
            }
            ...
            String ack = null;
            if (ConditionalCompilationControls.TESTHARNESS) {
                final String SYNCRESPONSECOUNTDOWN = "uk.nhs.digital.mait.spinetools.spine.syncresponsecountdown";
            ...
            if (soapAction == null) {
                SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.noSoapAction", "SOAPaction not found in received message");
                ...
                if (soapAction.contains(EBXMLACK) || soapAction.contains(EBXMLERROR)) {
                    ...
                    if (ackedId == null) {
                        SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.noAckedId", "Could not resolve RefToMessageId in received asynchronous acknowledgment");
                    } else {
                        cm.registerAck(ackedId, soapAction, message);
                    }
                    ...
                    if (ConditionalCompilationControls.TESTHARNESS) {
                        final String SOAPFAULT = "uk.nhs.digital.mait.spinetools.spine.connection.soapfault";
                    ...
                    if (isSpineSOAP(soapAction)) {
                        ...
                        if (ConditionalCompilationControls.TESTHARNESS) {
                            final String NEGEBXMLACK = "uk.nhs.digital.mait.spinetools.spine.connection.negativeebxmloverride";
                        ...
                        if (ebxmlException == null) {
                            ...
                            String ecode = "1000";
                            String ecodecontext = ebxmlException.getMessage();
                            String edesc = "ebXml Parser";
                            if (ConditionalCompilationControls.TESTHARNESS) {
                                final String NEGEBXMLACK = "uk.nhs.digital.mait.spinetools.spine.connection.negativeebxmloverride";
                                final String NEGEBXMLACKECODE = "uk.nhs.digital.mait.spinetools.spine.connection.negativeebxmloverride.ecode";
                                final String NEGEBXMLACKECODECONTEXT = "uk.nhs.digital.mait.spinetools.spine.connection.negativeebxmloverride.ecodecontext";
                                final String NEGEBXMLACKEDESC = "uk.nhs.digital.mait.spinetools.spine.connection.negativeebxmloverride.edesc";
                                String prop = System.getProperty(NEGEBXMLACK);
                                if (prop != null && prop.trim().toLowerCase().equals("y")) {
                                    ecode = System.getProperty(NEGEBXMLACKECODE);
                                    ecodecontext = System.getProperty(NEGEBXMLACKECODECONTEXT);
                                    edesc = System.getProperty(NEGEBXMLACKEDESC);
                                }
                            }
                            ack = msg.makeEbXmlNack(ecode, ecodecontext, edesc);
                        }
                        ...
                        if (handler != null) {
                            ...
                            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.noHandler", "Could not resolve message handler, and error initialising the default one");
                        }
                    }
                }
            }
            ...
            if (!socket.isClosed()) {
                try {
                    doSynchronousResponse("HTTP/1.1 500 Internal Server Error\r\nContent-Length: 0\r\n\r\n", "");
                    socket.close();
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.run-clientnotified", e);
                } catch (Exception eLast) {
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.run-notifyingclient", e);
                }
            } else {
                SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.run-client-not-notified", e);
            }
        }
    }
    ...
    private void doSynchronousResponse(String r, String a)
        ...
        if (ConditionalCompilationControls.TESTHARNESS) {
            if (ConditionalCompilationControls.otwMessageLogging) {
                ...
                SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.message", "\r\nReference to Message ID " + originatingMessageId + " - ON THE WIRE SYNC OUTBOUND: \r\n\r\n" + r);
                if (a != null && !a.equals("")) {
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.message", "\r\n\r\n" + a);
                }
            }
    ...
    private void doAsynchronousAck(String ack) {
        ...
        try {
            ...
            try (Socket s = tlsContext.createSocket(u.getHost(), (u.getPort() == -1) ? u.getDefaultPort() : u.getPort())) {
                ...
                if (rlength == -1) {
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.doAsynchronousAck", "Failed to get HTTP response to asynchronous ack");
                }
            }
        } catch (Exception e) {
            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SpineMessageHandler.doAsynchronousAck", e);
        }
