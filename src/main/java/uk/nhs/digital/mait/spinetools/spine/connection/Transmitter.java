    ...
    public void run() {
        ...
        if (!sendable.recordTry()) {
            if (sendable.getMessageId() != null) {
                c.removeRequest(sendable.getMessageId(), "RETRY COUNT: Transmitter.run()");
                sendable.expire();
            }
            return;
        }
...
//        SSLSocketFactory sf = tlsContext.getSocketFactory();
        ...
        try {
            if (h == null) {
                ...
                //
                host = ((uk.nhs.digital.mait.spinetools.spine.messaging.EbXmlMessage) sendable).getHost();
            } else {
                sendable.persist();
                URL u = new URL(h);
                host = u.getHost();
            ...
            try (Socket s = tlsContext.createSocket(host, port)) {
                ...
                if (sc == null) {
                    ...
                    if (replyLength == -1) {
                        SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.Transmitter.noResponse", "Could not read response sending " + sendable.getMessageId());
                        s.close();
                        return;
                    }
                    if (replyLength >= 0)
                    ...
                    if (replyLength >= 0) {  // even if no reply require the http header
            ...
            if (sendable.getType() == Sendable.SOAP) {
                if (sendable.getSynchronousResponse() == null) {
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.Transmitter.noResponseReceived", "No response to " + sendable.getMessageId());
                    return;
                }
            ...
            if (sendable.getMessageId() != null) { // Don't do this for asynchronous acks
                if (sendable.getSynchronousResponse() != null) {
                    if ((responseHeader != null) && (responseHeader.contains("HTTP 5"))) {
                        SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.Transmitter.HTTP500received", "HTTP 500 received sending " + sendable.getMessageId());
                        c.removeRequest(sendable.getMessageId(), "FAILED: HTTP 500 Transmitter.run()");

                    } else {
                        if (sendable.getSynchronousResponse().contains(sendable.getMessageId())) {
                            c.registerAck(sendable.getMessageId(), "OK. Synchronous ack", sendable.getSynchronousResponse());
                        }
                        if (sendable.getSynchronousResponse().contains("Bad request")) {
                            c.registerAck(sendable.getMessageId(), "FAILED: HTTP 400 Bad Request", sendable.getSynchronousResponse());
                            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.Transmitter.HTTP500received", "Bad request received sending " + sendable.getMessageId());
                        }
                    }
                }
            }
        } catch (Exception eIo) {
            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.Transmitter.IOException", "IOException sending " + sendable.getMessageId() + eIo.getMessage());
        }
    }
