       ...
       messageid = UUID.randomUUID().toString().toUpperCase();
        synchronized (SOAPREQUESTTEMPLATE) {
            if (template == null) {
                ...
                try {
                    BufferedReader br = new BufferedReader(new InputStreamReader((uk.nhs.digital.mait.spinetools.spine.messaging.SpineHL7Message.class.getResourceAsStream(SOAPREQUESTTEMPLATE))));
                    String line = null;
