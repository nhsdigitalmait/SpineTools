...
/**
 * The interface to locally-cached SDS endpoint details.<BR><BR>
 *
 * The cache is indexed in the first instance by service/interaction (with the
 ...
 *
 * This structure was chosen because it yields easily-processed, text files that
 * can be hand- edited if required, and that will work with external "cache
 * management" mechanisms. Also (at least from the perspective of the C#/.Net
 * flavour of the MHS, which was elaborated before the Java one), JSON is easy
 * to serialise and read due to the availability of an open source interface
 * that uses reflection to figure out how to do it for an arbitrary class. The
 * Java version is explicitly written to match the C# files and to make them
 * compatible with one another. However such compatibility is not guaranteed if
 * the files are hand-edited in place.<BR><BR>
 *
 * The cache implements an in-memory copy of the cached SdsTransmissionDetails
 * objects, backed by the JSON files on disk. This is read when the SdsCache
 * instance is constructed. A cache refresh (including to load details that have
 * been hand-edited in one or more cache files) can therefore be accomplished by
 * instantiating a new cache.<BR><BR>
 *
...
public class SdsCache {
    ...
    /**
     * Save the given SdsTransmissionDetails instance in the cache directory, in
     * JSON representation.
     */
    ...
    private void writeDetails(SdsTransmissionDetails sds) {
        ...
        try {
            ...
            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SdsCache.writeDetails", e);
        }
    ...
    /**
     * Makes a JSON object by serialising the SdsTransmissionDetails in the same
     * order as the JSON.Net does in the C# equivalent.
     *
     * @param sds
     * @return
     */
    ...
    private void loadInteraction(File cdir) {
        ...
        for (File f : files) {
            ...
            try {
                ...
                SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SdsCache.loadInteraction", e);
            }
    ...
    private SdsTransmissionDetails readDetails(JsonObject j) {
        ...
        try {
            ...
            SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.connection.SdsCache.readDetails", ecc);
        }
        return sds;
    }
     ...
     *
     * @param svcint "SvcIA" value consisting of TMS service name and HL7
     * interaction id
