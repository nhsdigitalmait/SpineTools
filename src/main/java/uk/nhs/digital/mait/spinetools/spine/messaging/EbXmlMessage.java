...
import uk.nhs.digital.mait.spinetools.spine.connection.ConditionalCompilationControls;
        ...
        if (startPosition == -1) {
            ...
            if (ctype.toLowerCase().startsWith("text/xml")) {
                if (soapAction.contains("Acknowledgment")) {
                    ...
                    if (a == null) {
                        SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.messaging.EbXmlMessage.parseError", "Failed to extract message id reference from Acknowledgment");
                        return;
                    }
                    ConnectionManager cm = ConnectionManager.getInstance();
                    cm.registerAck(a, soapAction, msg);
                    return;
                }
                if (soapAction.contains("MessageError")) {
                    ...
                    String a = EbXmlAcknowledgment.getAckedMessageId(msg);
                    if (a == null) {
                        SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.messaging.EbXmlMessage.parseError", "Failed to extract message id reference from MessageError");
                        return;
                    }
                    SpineToolsLogger.getInstance().log("uk.nhs.digital.mait.spinetools.spine.messaging.EbXmlMessage.MessageError", "MessageError received for " + a);
                    ConnectionManager cm = ConnectionManager.getInstance();
                    cm.registerAck(a, soapAction, msg);
                    return;
                }
