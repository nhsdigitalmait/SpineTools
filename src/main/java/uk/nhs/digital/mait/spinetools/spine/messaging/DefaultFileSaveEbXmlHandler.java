...
/**
 * Default implementation of SpineEbXmlHandler that writes a received message to
 * disk. An instance of this handler is created by the Connection Manager and
 * used where no other has been explicitly mapped for a service/interaction. It
 * is provided partly as an example, and partly as a simple means of integrating
 * Spine ebXml messaging with legacy systems.
 *
 * Note that this class has no link to any original ebXml request that the MHS
 * may have sent, and does not attempt to correlate received and sent messages
 * (hence it is equally applicable to receiving asynchronous responses, and
...
public class DefaultFileSaveEbXmlHandler
     ...
     */
    private static final String SAVE_DIRECTORY = "uk.nhs.digital.mait.spinetools.spine.messaging.defaultebxmlhandler.filesavedirectory";
     ...
     */
    private static final String REPORT_FILENAME = "uk.nhs.digital.mait.spinetools.spine.messaging.defaultebxmlhandler.reportfilename";
    ...
    public DefaultFileSaveEbXmlHandler()
        ...
        if (!fileSaveDirectory.exists()) {
            if (!fileSaveDirectory.mkdir()) {
                throw new FileNotFoundException("Default ebXml handler save location ("+fileSaveDirectory+") does not exist");
            }
        }
        if (!fileSaveDirectory.isDirectory()) {
            throw new IllegalArgumentException("Default ebXml handler save location ("+fileSaveDirectory+") must be a directory");
        }
        if (!fileSaveDirectory.canWrite()) {
            throw new IllegalArgumentException("Default ebXml handler save location ("+fileSaveDirectory+") must be writable");
        }
