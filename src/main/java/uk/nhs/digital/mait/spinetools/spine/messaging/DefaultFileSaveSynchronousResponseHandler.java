...
/**
 * SynchronousResponseHandler that writes the response to a file on disk. The
 * ConnectionManager will instantiate either a
 * DefaultFileSaveSynchronousResponseHandler or a NullSynchronousResponseHandler
 * for use when no explicit handler is mapped against a request type.
 *
 * Note that this class just saves the response, which is stored in the
 * SpineSOAPRequest in any case. Integration tasks that just need to get a
 * response back from Spine should use the NullSynchronousResponseHandler or a
 * subclass if it, unless they need to maintain a copy of the response (in which
...
public class DefaultFileSaveSynchronousResponseHandler
    ...
    /**
     * System property. Holds the path where responses are written. Note that if
     * this property is not set, the system "user.dir" property is used.
     */
    private static final String SAVE_DIRECTORY = "uk.nhs.digital.mait.spinetools.spine.messaging.defaultsynchronousresponsehandler.filesavedirectory";
     ...
     */
    private static final String REPORT_FILENAME = "uk.nhs.digital.mait.spinetools.spine.messaging.defaultsynchronousresponsehandler.reportfilename";
    ...
    public DefaultFileSaveSynchronousResponseHandler()
        ...
        if (!fileSaveDirectory.exists()) {
            if ( !fileSaveDirectory.mkdir()){
                throw new FileNotFoundException("Default synchronous response handler save location ("+fileSaveDirectory+") does not exist");
            }
        }
        if (!fileSaveDirectory.isDirectory()) {
            throw new IllegalArgumentException("Default synchronous response handler save location ("+fileSaveDirectory+") must be a directory");
        }
        if (!fileSaveDirectory.canWrite()) {
            throw new IllegalArgumentException("Default synchronous response handler save location ("+fileSaveDirectory+") must be writable");
        }
